...
workflow bamMergePreprocessing {
    ...
    meta {
        ...
        output_meta: {
          ...
          markDuplicateMetricsZip: "A tarball of markDuplicates metrics files across all intervals (if doMarkDuplicates enabled).",
          bamMetricsZip: "A tarball of samtools stats/flagstat/mapcounts metrics files generated at each processing stage (if doBamMetrics enabled)."
        }
    }
...
task bamMetrics {
  ...
  parameter_meta {
    inputBam: "Bam file to generate stats, flagstat, and mapcounts for."
    jobMemory: "Memory allocated to job (in GB)."
    cores: "The number of cores to allocate to the job."
    timeout: "Maximum amount of time (in hours) the task can run for."
    modules: "Environment module name and version to load."
  }
}
...
task subsetAndFilter  {
  ...
  parameter_meta {
    ...
    ncBed: "Bed file of non-canonical contigs, used to subset when interval is 'NC'."
    filterSuffix: "Suffix to append to output file name when filtering is applied."
    reference: "Path to reference file."
    jobMemory:  "Memory allocated to job (in GB)."
    minMemory: "A minimum amount of memory allocated to the task, overrides the scaled RAM setting"
    overhead: "Java overhead memory (in GB). jobMemory - overhead == java Xmx/heap memory."
    oldStyle: "Hidden option to revert to the previous dupMarking strategy, for assessment purposes."
    cores: "The number of cores to allocate to the job."
...
task splitNCigarString {
  ...
  parameter_meta {
    inputBam: "Bam file to split."
    inputBamIndex: "Index file for input bam."
    outputFileNamePrefix: "Output files will be prefixed with this."
    refactorCigarString: "SplitNCigarReads refactor-cigar-string option."
    splitNCigarReadsAdditionalParams: "Additional parameters to pass to GATK SplitNCigarReads."
    readFilters: "Optional GATK read filters to apply."
    reference: "Path to reference file."
    jobMemory: "Memory allocated to job (in GB)."
    overhead: "Java overhead memory (in GB). jobMemory - overhead == java Xmx/heap memory."
    cores: "The number of cores to allocate to the job."
...
task mergeBams {
  ...
  parameter_meta {
    ...
    jobMemory: "Memory allocated to job (in GB)."
    overhead: "Java overhead memory (in GB). jobMemory - overhead == java Xmx/heap memory."
    comment: "Comment to add to the header of the merged bam file."
    cores: "The number of cores to allocate to the job."
    timeout: "Maximum amount of time (in hours) the task can run for."
    modules: "Environment module name and version to load (space separated) before command execution."
  }
